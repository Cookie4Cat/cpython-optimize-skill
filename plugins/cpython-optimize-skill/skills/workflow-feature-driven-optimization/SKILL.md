...
description: Use when 已知 CPython/CinderX 特性或实现路径预计能提升性能，需要修改代码、补 correctness 用例并验证 pyperformance 收益。
---
...
| correctness | `cinderx-jit-analyst` / orchestrator | `cpython-runtime-test-run`、`cinderx-smoke-check` |
...
先功能后性能。L1 correctness 未通过时，不讨论性能收益；目标 benchmark 无收益时，先解释假设失败原因。
