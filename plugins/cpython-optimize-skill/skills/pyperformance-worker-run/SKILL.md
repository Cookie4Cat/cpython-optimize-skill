...
- CinderX worker 必须能看到系统 site-packages：确认 `include-system-site-packages = true`，或用等价 `PYTHONPATH` 把系统 site-packages 放进 worker 环境。
...
- worker import 成功但 driver 失败：查 pyperformance venv、`include-system-site-packages`、`PYTHONPATH` 和 `--inherit-environ`。
...
- 性能数据异常慢：先确认 CPU 绑核、JIT flags、debug dump、系统 site-packages 和 baseline/candidate 唯一差异轴。
